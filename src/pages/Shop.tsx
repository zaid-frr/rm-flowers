...
export default function Shop() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        let prods = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as any));
        
        const params = new URLSearchParams(location.search);
        const category = params.get('category');
        
        if (category) {
          prods = prods.filter(p => p.category?.toLowerCase() === category.toLowerCase() || p.category?.toLowerCase().includes(category.toLowerCase()));
        }
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="animate-pulse text-deep-leaf font-serif text-xl">Loading elegant arrangements...</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <h1 className="text-4xl md:text-5xl font-serif font-bold text-deep-leaf mb-4 text-center">
        {category ? `${category.charAt(0).toUpperCase() + category.slice(1).replace('-', ' ')} Collection` : 'Shop All'}
      </h1>
      <p className="text-center text-ink/70 mb-12 max-w-2xl mx-auto">
        {category 
          ? `Explore our curated selection of ${category.replace('-', ' ')} arrangements.`
          : 'Discover our complete collection of meticulously crafted artificial floral arrangements, designed to bring everlasting beauty to your space.'}
      </p>
      
      {products.length === 0 && !loading ? (
        <div className="text-center py-16">
          <p className="text-xl text-ink/60 font-serif">No products found in this category.</p>
